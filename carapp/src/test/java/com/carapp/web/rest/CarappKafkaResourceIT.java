...
class CarappKafkaResourceIT {
    ...
    void setup() {
        ...
        CarappKafkaResource kafkaResource = new CarappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/carapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/carapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
